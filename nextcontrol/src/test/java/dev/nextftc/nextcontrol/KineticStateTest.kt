...
import dev.nextftc.nextcontrol.test.kineticStateGen
...
class KineticStateTest : FunSpec({
    ...
    context("Property-based testing") {
        test("plus is commutative") {
            checkAll(kineticStateGen, kineticStateGen) { a, b ->
                val result1 = a + b
                val result2 = b + a
        ...
        test("plus and minus are inverse operations") {
            checkAll(kineticStateGen, kineticStateGen) { a, b ->
                ...
                result.velocity shouldBe a.velocity
                result.acceleration shouldBe a.acceleration
            }
        }
        
        test("multiplying by 1 doesn't change the state") {
            checkAll(kineticStateGen) { state ->
        ...
        test("multiplying by 0 results in all zeros") {
            checkAll(kineticStateGen) { state ->
                ...
                result.position shouldBe 0.0
                result.velocity shouldBe 0.0
                result.acceleration shouldBe 0.0
            }
        }
    }
})