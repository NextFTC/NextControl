...
class PIDElementTest : AnnotationSpec() {
    ...
    fun `returns error when kP is one and kI and kD are zero`() {
        ...
        // Assert
        positionActual shouldBe error
        velocityActual shouldBe error
    }
    ...
    fun `returns sum of all past errors times deltaT when kI is one and kP and kD are zero`() {
        ...
        val positionPID = PIDElement(FeedbackType.POSITION, coefficients, timeSource = timeSource)
        val velocityPID = PIDElement(FeedbackType.VELOCITY, coefficients, timeSource = timeSource)
    ...
    fun `acceleration error does not affect position pid`() {
        ...
        val firstController = PIDElement(FeedbackType.POSITION, coefficients, timeSource = timeSource)
        val secondController = PIDElement(FeedbackType.POSITION, coefficients, timeSource = timeSource)
        ...
        // Assert
        secondActual shouldBe firstActual
    }
    ...
    fun `position error does not affect velocity pid`() {
        ...
        val firstController = PIDElement(FeedbackType.VELOCITY, coefficients, timeSource = timeSource)
        val secondController = PIDElement(FeedbackType.VELOCITY, coefficients, timeSource = timeSource)
        val firstError = KineticState(10.0, 20.0, 30.0)
